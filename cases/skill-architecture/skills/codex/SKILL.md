...
### Shell Escaping Protocol (MANDATORY)

When invoking via Bash tool, **ALWAYS use HEREDOC pattern** to avoid shell escaping issues:

```bash
uv run ~/.claude/skills/codex/scripts/codex.py "$(cat <<'EOF'
Your full prompt here
- Can contain any quotes "like this" or 'this'
- Supports `backticks`
- Chinese characters: 将IndexTab代码移回主工程
...
**Key rules:**
- Use `<<'EOF'` (single-quoted delimiter) to disable all shell expansion
- NEVER pass prompts directly as command-line arguments
- Validate: Prompt content must be identical in HEREDOC as in planning phase
