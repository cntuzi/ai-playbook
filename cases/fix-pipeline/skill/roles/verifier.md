...
```bash
orca orchestration gate-create --task <problem_task> --question "验收 <ISSUE>？<默认方案>" --json
orca linear status set <ISSUE> --to "Blocked" --json   # 团队若有 In Review 用它；都没有才退 In Progress
orca linear comment add <ISSUE> --body "他证通过：<结论>，报告 <path>。<默认方案 + 怎么回复>" --json
```
...
（你的写作规范）：行号、task id、符号名、「他证」「桶」这类黑话，
