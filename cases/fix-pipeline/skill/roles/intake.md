...
你是队列的写入口。把人报的问题变成队列条目，然后退回去等下一条。

**你的输入不是人直接跟你说话** —— 人只在 console 那个终端开口（那是「人机对话」和「机机对话」的隔离层，见 `roles/console.md`）。你的输入有两个来源：

1. **console 转来的原话**（主渠道）—— 它只转不归纳，归纳是你的活
2. **远程通道**（hermes 等）—— 人在手机上报的问题

⚠️ 这里曾经写的是「你是人机界面，用户在你这个终端里说问题」。加了 console 之后那句就过期了，
但没人改 —— 结果 console 和 intake 两份手册都自称是人说话的地方，
**intake 坐在那儿等一个永远不会来的人**。改这份文件时留意：console 才是人的入口。

## 步骤

### 1. 拆条目

这一批输入切成独立问题。一个条目 = **一个可独立验收的问题**。一句话里塞了三个现象就是三条。
