# Fix Pipeline —— 队列驱动的多 Agent 修复闭环

一批问题进去，修好的出来。每个修复都由**另一个** agent 独立复验过，再由人在看板上验收。

**技术栈：** Orca orchestration（任务队列、dispatch、decision gate）+ 任意 TUI 编码 agent
（Claude Code / Codex / omp / Gemini）+ Linear 作为人的验收看板。

> **状态：** 已跑通一轮完整闭环（入队 → 分桶 → 修复 → 他证 → 人验收）。编排层假设已用探针实测（[design.md](./design.md) 第八节）。
> 下面「测试测出了什么」记录了测试推翻设计的那一处。
> 这个仓库记录跑得起来的系统，所以也如实说明哪些部分还没被证明。

---

## 问题在哪

把十个 bug 丢给一个 agent，你会拿到十个看起来都挺像回事的修复。有些是真的。有些只治了症状。
有些压根不是 bug —— agent 自己幻觉出来的。而告诉你"修好了"的，正是写这个修复的同一个 agent。

规模化这件事需要的不是更多并行，而是一个让**「完成」必须被挣三次**的结构。

## 四个概念撑起全部

**队列是真源。** 每个问题是编排任务队列里的一行。agent 之间的消息只是唤醒信号 —— 丢一条不影响
正确性，因为任何 agent 都能从队列里恢复自己该干的活。队列丢了才是全盘皆输。这就是常驻 agent
能随时重启的原因。

**决定并发度的是桶，不是问题数。** 一个桶 = 一组 touched-files 互不重叠的问题，交给一个 agent
在一个位置处理。按**碰哪些文件**聚类而不是按问题类别（崩溃 / UI / 性能）聚类，是并行合并零冲突的
前提 —— 类别跟 merge 冲突毫无关系，它另有用途：决定派哪个模型。

**三证链。** 自证（修复 agent 说改完了）→ 他证（**另一个**只读 agent 复验）→ 人证（人在看板上
验收）。少一环，绿灯就没有意义。他证必须换 agent：一个 agent 验自己的活，不叫验证。

**回边。** 他证不过、人打回，问题重新入队等人领。这条边让它是循环，不是流水线。

## 形状

```
intake ──▶ [队列] ──▶ analyzer ──▶ 桶 ──▶ fixer ──▶ 他证 ──▶ gate ──▶ 人
             ▲                                                        │
             └──────────── 打回：重新入队 ◀─────────────────────────┘
```

六个角色，每个都是一个普通 TUI agent 会话，只读它自己那份角色文件：

| 角色 | 干什么 | 存在形式 |
|---|---|---|
| **console** | 人的唯一入口：答人的提问、维护手册。不领队列任务 | 常驻 |
| **intake** | 接问题、归一化、去重、写队列 | 常驻 |
| **analyzer** | 按 touched-files 分桶、建任务链、派活 | 常驻 |
| **fixer** | 串行修一个桶，跑自证小循环 | 每桶一个，可复用 |
| **verifier** | 拉待验证任务、派他证、请人验收 | 常驻 |
| **watchdog** | 重启掉线的常驻角色，它们自己从队列找回活 | 常驻 |

`console` 是跑起来之后才长出来的角色。角色之间用 `orchestration send` 通信，而这些消息会被**原文注入**到收信终端的对话里 —— 人原本坐在消息汇聚点那个终端，于是「归属别记错」「探针漏报」这类内部往返全糊在他眼前。console 把人机对话和机机对话拆成两个终端，这是它存在的全部理由。

### 用 worktree 父子关系统筹

能力跨项目，但每次运行**绑定一个项目上下文** —— 在哪个项目下启动就服务哪个项目，这个绑定用 worktree lineage 表达：

```
<项目 worktree>
 └── fix-pipeline          控制面（child，带项目代码）
      │  └─ 常驻角色的终端都在这里
      ├── <bucket-a>       桶（控制面的 child）
      │     ├─ fixer
      │     └─ 他证 agent（只读，同 worktree）
      └── <bucket-b>
```

控制面必须**带项目代码** —— analyzer 要读项目文件才能按 touched-files 分桶。这条是踩出来的：控制面最初建在一个语义无关的空仓库里，他证 agent 在空目录里打转，analyzer 更是根本没法干活。

lineage 同时是**通信拓扑**：worker 读自己的 `parentWorktreeId` 就能找到控制面，再列终端找 coordinator。terminal handle 随重启变化，父子关系不变 —— 这解掉了「coordinator 重启后在途 `worker_done` 发到死 handle」那个悬案。

## 状态模型为什么长这样

编排层的任务状态**只有六个且不可扩展**：`pending / ready / dispatched / completed / failed /
blocked`。而「待验证」和「待验收」两个业务态无处安放。

与其另建一套并行状态存储，两者都用**结构**表达：

- **待验证** → 一个 `--deps` 依赖修复任务的验证任务。修复完成，它自动变成可独立领取。
- **待验收** → 一个卡住父任务的 decision gate。gate 列表**本身**就是待验收队列。

结果比自定义状态更干净，而且这两种都是编排层原生在用的模式。

## 测试测出了什么

用一次性探针 task 把编排层的假设实测了一遍（探针已清理）。两条按预期成立：`--ready` 确实严格按
`deps` 满足过滤，`gate-create` 确实把 task 置成 `blocked`。

但有一条**推翻了设计**：

```
新建无依赖的 task  →  直接是 ready，不经过 pending
pending 的真实含义 =  「依赖未满足」，不是「刚入队」
```

后果是致命的 —— skill 里 analyzer 的第一步原本写的是 `task-list --status pending`，而 intake
建的问题 task 根本不会是 `pending`。**照着跑，analyzer 每轮拿到空列表，整条流水线一动不动。**

顺着这条又揪出两个断点：问题 task 分完桶如果不显式置 `dispatched`，下一轮会被重复分桶重复派
worker；打回时新建的重试 task 如果挂了 `--parent`，`parent_id` 非空会让 analyzer 的筛选漏掉它，
问题永久卡死在队列里。

三处都已修。这一段留在这里，是因为它比修好之后的正确版本更有信息量：**看起来最稳的那类假设
（一个状态字段的含义），恰恰是最容易想当然、也最容易整条链路静默失效的地方。**

## 内容

- **[design.md](./design.md)** —— 完整设计：状态映射、八条纪律、
  **已否决方案及理由**、待验证假设、已知风险
- **[skill/](./skill/)** —— 可跨 agent 使用的 skill 实物：共享契约 + 每角色一个文件

## 迁移到别处时注意

选 Linear 当人的看板，是因为编排层没有问题粒度的 UI —— 它的 task 和 gate 列表只有 CLI。
任何有可编程 API 的看板都行；关键是**只有一套存储对机器权威**，看板是**投影**，且人只有一个
可写字段（通过 / 打回）。两套状态存储双向同步，就是漂移的来源。

原语的真源是官方 guide（`orca skills get orchestration`）。这个案例只记录它之上的工作流层，
刻意不复述命令语法 —— 这样它不会跟你手上那个 CLI 版本漂移。
