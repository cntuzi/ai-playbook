...
```bash
orca linear comment add <ISSUE-n> --body "补充复现：<新证据>" --json
```
...
实测 —— ISSUE-4 正文写的判据是第 1 轮的 toast 方案，该方案在人工验收时已被否决，正文从没跟着改。
...
都在被它带偏。实测：ISSUE-4 从方向被否到正文被改隔了一整条修复链，期间只能靠评论贴
...
orca linear save-issue ISSUE-n --description "<原 body>

orchestration task: task_xxx" --json
```

顺序反了的补救：把 id 塞进 `task-update --result '{"linear":"ISSUE-n"}'`。能用，但 `result` 的语义是 worker 完成回执，塞交叉引用是脏的 —— 只当补救，别当常规。
...
orca orchestration send --to "@worktree:<wf-id>" --type status \
  --subject "新问题入队" --body "task_xxx / ISSUE-n" --json
```
